...
**VSCode terminal** (`~/dev/ascent/src/vs/workbench/contrib/terminal/`) — gold standard for xterm.js usage:
- `browser/xterm/xtermTerminal.ts` — xterm.js wrapper, renderer selection, addon management
- `browser/terminalInstance.ts` — write path, data segmentation, flush handling
- `browser/terminalProcessManager.ts` — flow control, ack batching, process lifecycle
- `common/terminalConfiguration.ts` — all terminal config options

**codemirror-rich-markdoc** (`github.com/segphault/codemirror-rich-markdoc`) — reference for CM6 rich markdown editing (Obsidian-style):
- `src/richEdit.ts` — decoration plugin pattern: collect `Range<Decoration>[]`, use `Decoration.set(widgets)` for auto-sorting
- `src/renderBlock.ts` — block widget replacements for complex elements
- Uses `cm-markdoc-hidden` CSS class pattern for hiding syntax characters
...
│   ├── agent-config/              # Agent config pane components
│   │   ├── AgentConfigPane.tsx       # Main pane container with editor
│   │   └── AgentConfigSidebar.tsx    # Sidebar listing config files by agent
...
├── lib/
│   ├── agent-config.ts        # Agent config file definitions per tool
...
### AgentConfigPane
Dedicated pane for viewing/editing AI agent config files (CLAUDE.md, AGENTS.md, .cursorrules, etc.). Toggle via Cmd+Shift+A or sidebar bot icon. Features:
- **Sidebar**: Config files grouped by agent (Claude, Codex, Cursor, Copilot, etc.) with existence indicators
- **Create**: One-click creation of missing config files with sensible defaults
- **Editor**: Reuses MarkdownEditor for .md files, CodeEditor for .json — same editors as EditorPane
- Config file definitions live in `src/lib/agent-config.ts` (easy to extend with new agents/files)
